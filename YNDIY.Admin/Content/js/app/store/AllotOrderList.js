require(['/Content/minjs/config.js'], function () {
    require(['jquery', 'pop_window', 'base', 'laydate', 'domReady'], function ($, _, base, _, _) {
        ...
        window._load_items = function (_page_index) {
            $.when(base.loadHtmlData({ status: $("#status").val(), pageIndex: _page_index, start_data: $('#start_date').val(), end_data: $('#end_date').val() }, "/ERPStore/GetallotOrderList")).done(function (res) { $("._ajax_container").empty().append(res); base.repainWindow(); });
        }
        _load_items(1);
