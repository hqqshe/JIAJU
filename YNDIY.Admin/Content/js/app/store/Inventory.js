require(['/Content/minjs/config.js'], function () {
    require(['jquery', 'pop_window', 'base', 'laydate', 'domReady'], function ($, _, base, _, _) {
        ...
        window._load_items = function (_page_index) {
            $.when(base.loadHtmlData({searchKey: $('.search_input').val(), pageIndex: _page_index}, "/ERPStore/GetInventoryList")).done(function (res) { $("._ajax_container").empty().append(res); base.repainWindow(); });
        }
        _load_items(1);
